...
void menu_show() {
	...
	log("show_menu: hiding home screen\n");
	sceImposeSetHomePopup(home_enabled);
	sceImposeHomeButton(0);
		...
		if(search.num_results) {
			...
			for(i = start; i < end; i++) {
				...
				if(menu.cheater.edit_format) {
					...
					// print out the float
					f_cvt((unsigned int *)search.results[i], buffer, sizeof(buffer), 6, MODE_GENERIC);
					puts(buffer);
				}
...
void layout_modinfo(SceUID uid) {
	...
	if(mod_info) {
		...
		ctrl_waitmask(0, 0, PSP_CTRL_CIRCLE);
		...
		for(i = 0; i < 26; i++) {
			if(menu.bd.view) {
				...
				if(menu.bd.decoder_format) {
					...
					// print out the float
					f_cvt((unsigned int *)(menu.bd.decoder[menu.bd.no].address + (i * 4)), buffer, sizeof(buffer), 6, MODE_GENERIC);
					puts(buffer);
				} else {
					// print out the opcode
